...
local_server=True
app=Flask(__name__)
app.secret_key="nishanthrkashyap"
...
app.config['SQLALCHEMY_DATABASE_URI']='mysql://root:Nishanthsep#3@localhost/coviddbms'
...
def signup():
    if request.method=="POST":
        ...
        # sqlcommand=text(f"INSERT INTO `user` (`srfid`,`email`,`dob`) VALUES ('{srfid}','{email}','{encpassword}') ")
        # new_user=db.engine.connect().execute(sqlcommand)
...
def login():
    if request.method=="POST":
        ...
        dob=request.form.get('dob')
        user=User.query.filter_by(srfid=srfid).first()
        # print('user',type(user.dob),'dob',type(dob))
        # print(check_password_hash(user.dob,dob))
...
def hospitallogin():
    if request.method=="POST":
        print('current1',current_user);
        email=request.form.get('email')
        password=request.form.get('password')
        user=Hospitaluser.query.filter_by(email=email).first()
        print('user',user.password,'password',password)
        print('check',check_password_hash(user.password,password))
        if user and check_password_hash(user.password,password):
            login_user(user)
            print('current2',current_user)
            flash("Login Success","info")
            return render_template("index.html")
        else:
            flash("Invalid Credentials","danger")
...
def hospitalUser():
    ...
    if('user' in session and session['user']==params['user']):
        ...
        if request.method=="POST":
            ...
            if  emailUser:
                flash("Email or srif is already taken","warning")
            # sqlcommand=text(f"INSERT INTO `hospitaluser` (`hcode`,`email`,`password`) VALUES ('{hcode}','{email}','{encpassword}') ")
            
            # db.engine.connect().execute(sqlcommand)
            query=Hospitaluser(hcode=hcode,email=email,password=encpassword)
            db.session.add(query)
            db.session.commit()
            # my mail starts from here 
           
            mail.send_message('Hospital CARE CENTER',sender=params['gmail-user'],recipients=[email],body=f"Welcome thanks for choosing us\nYour Login Credentials Are:\n Email Address: {email}\nPassword: {password}\n\nHospital Code {hcode}\n\n Do not share your password\n\n\nThank You..." )
...
def addhospitalinfo():
    ...
    if request.method=="POST":
        ...
        if huser:    
            # sqlcommand=text(f"INSERT INTO `hospitaldata` (`hcode`,`hname`,`normalbed`,`hicubed`,`icubed`,`vbed`) VALUES ('{hcode}','{hname}','{nbed}','{hbed}','{ibed}','{vbed}')")        
            # db.engine.connect().execute(sqlcommand)
